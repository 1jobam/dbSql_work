...
-- NOT NULL : 데이터 입력시 해당 컬럼에 값이 반드시 돌아와야 한다.
...
--해당 방법은 위의 두 가지 예씨 처럼 컬럼 레벨에서는 생성 할 수 없다.
--> TABLE LEVEL 제약 조건 생성

-- 기존에 생성한 dept_test 테이블 삭제
...
CREATE TABLE dept_test(
    deptno NUMBER(2),
    dname VARCHAR(14),
    loc VARCHAR(13), -- 마지막 컬럼 선언후 컴마 빼먹지 않기
...
INSERT INTO detp_test VALUES (98, 'ddit', '대전');
...
desc emp;
--empno, ename, deptno : emp_test 생성
--empno PRIMARY KEY 제약
...
--해당 데이터를 참조 하고 있는 EMP테이블의 DEPTNO 컬럼을 NULL로 변경 한다.
delete dept_test
WHERE deptno = 99;
ROLLBACK;
...
--emp_gb VARCHAR2(2) --직원 구분 01-정규직, 02인턴
...
--emp_gb check 조건에 맞추어 '02'로 변경시 정상 입력
...
select * from emp_test;
--기존 컬럼 수정
--ALTER TABLE 테이블명 MODIFY (컬럼 컬럼 타입);
--hp 컬럼의 타입을 VARCHAR2(20) -> VARCHAR2(30)
ALTER TABLE emp_test MODIFY (HP VARCHAR2(30));

--현재 emp_test 테이블에 데이터가 없기 때문에 컬럼 타입을 변경하는 것이 가능하다.
--hp 컬럼의 타입을 VHARCHAR2(30) -> NUMBER
ALTER TABLE emp_test MODIFY (HP NUMBER);
desc emp_test;

--컬럼명 변경
--해당 컬럼이 PK, UNIQUE, NOT NULL, CHACK제약 조건시 기술한 컬러명에 대해서도 자동적으로 변경이 된다.
--hp 컬럼 hp_n
--ATTER TABLE 테이블명 RENAME COLUMN 기존컬럼명 TO 변경컬럼명;
